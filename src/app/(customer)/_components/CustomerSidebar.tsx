...
export default function CustomerSidebar() {
  ...
  // Listen for hamburger events dispatched from topbar
  useEffect(() => {
        ...
        <div className="mt-auto px-3 border-t border-ds-outline-variant pt-4 space-y-0.5">
          ...
          {NAV_BOTTOM.map(({ label, href, icon }) => (
            <Link
              key={href}
              href={href}
              className="flex items-center px-3 py-2.5 type-body-sm text-ds-on-surface-variant hover:bg-ds-surface-container-low hover:text-ds-on-surface rounded transition-colors duration-150"
            >
              <span className="material-symbols-outlined mr-3 text-title-sm">{icon}</span>
              {label}
            </Link>
