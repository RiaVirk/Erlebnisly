...
const NAV_MAIN = [
  ...
  { label: "Schedule",   href: "/host/availability", icon: "calendar_month" },
  { label: "Bookings",   href: "/host/bookings",     icon: "confirmation_number" },
  { label: "Past Trips", href: "/host/earnings",     icon: "history" },
  { label: "Reviews",    href: "/host/experiences",  icon: "rate_review" },
] as const;
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative px-5 pt-6 pb-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-ds-on-surface-variant font-medium">Host Portal</p>
            </div>
          </div>
          <button
          ...
          {NAV_MAIN.map(({ label, href, icon }) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <span
                    className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-full bg-ds-primary"
                ...
                {active && (
                  <span
                    className="relative ml-auto w-1.5 h-1.5 rounded-full bg-ds-primary"
          ...
          {NAV_BOTTOM.map(({ label, href, icon }) => {
            ...
            return (
              <Link
                ...
                `}
              >
                {active && <span className="absolute inset-0 rounded-xl bg-gradient-to-r from-[rgba(255,77,0,0.12)] to-[rgba(255,77,0,0.04)]" />}
                {!active && <span className="absolute inset-0 rounded-xl bg-ds-surface-container-low opacity-0 group-hover:opacity-100 transition-opacity duration-150" />}
                {active && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-full bg-ds-primary"
                    style={{ boxShadow: "0 0 10px 2px rgba(255,77,0,0.55)" }} />
                )}
                <span className="relative material-symbols-outlined shrink-0" style={{ fontSize: 19, fontVariationSettings: active ? "'FILL' 1" : "'FILL' 0" }}>
                  {icon}
                </span>
                <span className="relative">{label}</span>
              </Link>
        ...
        {/* User account card */}
        <div
          className="mx-3 mb-5 mt-3 px-3.5 py-3 rounded-xl flex items-center gap-3"
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-ds-on-surface-variant truncate">Host profile</p>
          </div>
        </div>
      </aside>
    </>
